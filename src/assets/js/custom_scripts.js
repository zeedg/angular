...
//alert('Here');
    ...
    if (term.length > 1) {
        $.ajax({
            url: "global_search1.php?city=" + category + "&term=" + term, async: true, success: function (data) {
                result = data;
            }
        });
    }
    ...
    $(document).ready(function ($) {
        ...
        $('#useAddress').click(function () {
            ...
            if ($("input[name=ship_to_this]").is(':checked')) {
                pageurl = 'https://www.medicalshipment.com/index.php?controller=cart&function=checkout';
                var shipto = $('input:radio[name=ship_to_this]:checked').val();

                $("#loading_address").show();
                $("#slide_address").slideUp();
                $.ajax({
                    url: pageurl + '&ship_to_this=' + shipto, async: true, success: function (data) {
        ...
        $('#checkoutContinue').click(function () {
            ...
            pageurl = 'https://www.medicalshipment.com/index.php?controller=cart&function=review';
            ...
            if (paymentMethod == 'ccpayment' || paymentMethod == 'echeckpayment') {
                ...
                $.ajax({
                    method: "POST",
                    url: pageurl,
                    data: {payment_method: paymentMethod, cc_id: ccId},
                    async: true,
            ...
            if (paymentMethod == 'checkmo') {
                ...
                $.ajax({
                    method: "POST",
                    url: pageurl,
                    data: {payment_method: paymentMethod, checkmo: checkmo, poNo: poNo},
                    async: true,
        ...
        $('#amzChangeAddress').click(function () {
            pageurl = 'https://www.medicalshipment.com/index.php?controller=cart&function=payment';
            ...
            $("#loading_address").show();
            $.ajax({
                url: pageurl, async: true, success: function (data) {
                    $('body').html(data);
                }
            });
...
function return_cart() {
    window.location.href = 'https://www.medicalshipment.com/index.php?controller=cart&function=index';
}
...
function getCartItemsTop() {
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=ajax_cart_items", function (response) {
...
function varifyaddress() {
    ...
    $.ajax({
        url: 'https://www.medicalshipment.com/validate_address.php',
        global: false,
        type: 'POST',
        data: {address: address, zip: zip, city: city, state: state},
        cache: false,
...
function addCarthome(id) {
    ...
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=ajax", {
        product_id: id,
        cquantity: qnty,
        unit_id: unit_id
    }, function (data) {
...
function virtual_catalog() {
    $.post("https://www.medicalshipment.com/index.php?controller=catalog&function=virtual_catalog", function (data) {

    });
}

function get_uom(val) {
    $.post("https://www.medicalshipment.com/index.php?controller=product&function=get_uom", {cpo_id: val}, function (data) {
        $("#uom").html(data);
    });
}
...
function getCartItemAjax() {
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=ajax_cart", function (response) {
...
function add_shipping(ship, name) {
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=set_delivery", {
        ship: ship,
        name: name
    }, function () {
...
function AddCard() {
    ...
    $("#card_error").hide();
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=check_card", {
        cardnum: cardnum,
        cardname: cardName
    }, function (response) {
        //alert(response);
        if (response != 1) {
            ...
            $.post("https://www.medicalshipment.com/index.php?controller=cart&function=add_creditcard", {
                cardnum: cardnum,
                cardcvv: cardcvv,
...
function EditCard() {
    ...
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=check_card", {
        ...
        if (response != 1) {
            ...
            $.post("https://www.medicalshipment.com/index.php?controller=cart&function=edit_creditcard", {
                cardId: cardId,
                cardnum: cardnum,
                cardcvv: cardcvv,
                card_holder_name: cardHolderName,
...
function AddBank() {
    ...
    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=add_bank", {
...
function paymentMethod(method) {
    if (method == 'ccpayment') {
        ...
        $('#add_a_card').text(
            text == "Add a card" ? "Close" : "Add a card");
    ...
    if (method == 'checkmo') {
        ...
        $('#send_check').text(
            text == "Send Check / Money order to medicalshipment" ? "Close" : "Send Check / Money order to medicalshipment");
    ...
    if (method == 'echeckpayment') {
        ...
        $('#checking_account').text(
            text == "Add a checking account" ? "Close" : "Add a checking account");
...
function showEditCard(id) {
    ...
    var text = $('#add_a_card').text();
    $('#add_a_card').text(
        text == "Edit card" ? "Close" : "Edit card");

    $.post("https://www.medicalshipment.com/index.php?controller=cart&function=get_card", {id: id}, function (response) {
    ...
    var cont = "offlajn-universal-product-slider-90-1-container";
    var theme = "flat";
    var lArrow = $("#" + cont + " .off-uni-slider-left-container").on("click", function () {
        owl90.trigger("owl.prev");
    });
    var rArrow = $("#" + cont + " .off-uni-slider-right-container").on("click", function () {
        owl90.trigger("owl.next");
    });
    if (theme == "flat" || theme == "plastic" || theme == "modern" || theme == "light" || theme == "simple" || theme == "blank") {
        var lArrow = $("#" + cont + " .off-uni-slider-left").on("click", function () {
            owl90.trigger("owl.prev");
        });
        var rArrow = $("#" + cont + " .off-uni-slider-right").on("click", function () {
            owl90.trigger("owl.next");
        });
    }
...
$(document).ready(function () {
    ...
    $('#mainbutton').on("mouseenter", function () {
            hovering = true;
